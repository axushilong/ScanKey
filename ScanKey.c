...
//static  skwide_t skwContinue = 0;  /**< continue(继续、连续)按键按下保持的信号 */
//static  skwide_t skwTriger = 0;    /**< triger(触发)对应按键按下的信号,记录当前扫描是否有新的按键按下,有的话对应的位置1 */
//static  skwide_t skwReset = 0;

void vScanKeyRead(stScanKeyCtrlDataTypes * pstKeyCtrlData, stScanKeyTypes * pstSK){
	 skwide_t skwReadData = skwSKPortReadKeyVal();/**< 用于记录激活的按键数据,按键按下表示激活,对应位置1 */
	 skwide_t skwChange = skwReadData ^ pstKeyCtrlData->kwContinue;/**< 记录当前和上次不同状态的按键数据,对应位为1表示此按键电平对比上次有变动 */

	pstKeyCtrlData->kwContinue = skwReadData;/**< 保存本次激活的位 */

	pstKeyCtrlData->kwTriger = skwReadData & skwChange;/**< 当前激活的位是否包含,当前激活和上次激活的不同的位;也就是上次未激活,当前激活的位 */
	pstKeyCtrlData->kwReset = skwChange & (~kwReadData);/**< 记录由按下到抬起的触发 */
}

void vScanKey(stScanKeyCtrlDataTypes * pstKeyCtrlData, stScanKeyTypes * pstSK, unsigned char ucKeyCount){
	unsigned char ucI = 0;
	vScanKeyRead(pstKeyCtrlData,pstSK);
	for(ucI =0;  ucI < ucKeyCount; ucI++){
		...
		switch(pstSK[ucI].ucSKStatus){
			...
			default:{
				if(pstKeyCtrlData->kwReset & (1<<ucI)){/*对应按键动作抬起,复位*/
					if(pstSK[ucI].ucDebounceTick == 0){pstSK[ucI].ucDebounceTick = 1;/*开始抖动过滤检测*/}
					// pstSK[ucI].ucSKEvent = DE_SKEventUp0;/*抬起事件*/
					// DE_SKEventCallBack(&pstSK[ucI]);//pstSK[ucI].pfSKCallBack(&pstSK[ucI].ucSKEvent);
				}else if((~pstKeyCtrlData->kwContinue) & (1<<ucI)){/*对应按键保持抬起*/
					pstSK[ucI].ucDebounceTick++;
					if(pstSK[ucI].ucDebounceTick >= DE_SKDebounceTick){/*通过抖动验证*/
						pstSK[ucI].ucDebounceTick = 0;/*方便确认下次有没有经过抖动验证*/
						// pstSK[ucI].usDoubleStartTick = pstSK[ucI].usSKScanCount;/*用于双击抬起的时间判断*/
						pstSK[ucI].ucSKStatus = DE_SKStatusStart;
						pstSK[ucI].ucSKEvent = DE_SKEventNone;/*确认抬起0事件*/
						DE_SKEventCallBack(&pstSK[ucI]);//pstSK[ucI].pfSKCallBack(&pstSK[ucI].ucSKEvent);
					}else{/*按键是抬起的,但没有通过抖动事件,继续抖动验证*/
						;
					}
