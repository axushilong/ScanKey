...
static bool bIsOnlyClick(uxWide_t uxXBCapOnlyClick, uxWide_t uxXBValue){
    if(uxXBValue & (uxXBValue -1)){ //组合按键
        ...
        return true;  //定义了组合按键只支持单击
    #else
        return false; //组合按键默认支持单击、长按、Hold和多击
    #endif
    }else{ //非组合的单按键
...
static bool bIsEncodeMultiClick(uxWide_t uxXBCapEncodeMultiClick, uxWide_t uxXBValue){
    if(uxXBValue & (uxXBValue -1)){ //组合按键
        return false; //组合按键默认支持单击、长按、Hold和多击,不支持编码多击
    }else{ //非组合的单按键
        return (uxXBValue & uxXBCapEncodeMultiClick);
    }
}
...
void vXB(stXBTypes * pstXB){
    ...
    if(uxCurVal != pstXB->uxLastValue){ // 值变化
        if/*松开开始*/(uxCurVal == DE_NoPress){ // 变成无按键按下，即松开
            ...
            if(pstXB->ucClickCnt == pstXBCfg->ucClickCntMax){ //已经是最大多击次数时
                uiEvent = pstXB->ucMultiClickEncode;
                printf("xBEventNotify-%d: keyVal=%d, event=%d, clickCnt=%d\n", __LINE__, pstXB->uxNotifyValue, uiEvent, pstXB->ucClickCnt);
                goto _LabelXBEventNotify;
            }
    ...
    }else{ // 支持多击的按键，长按只标记到 ucMultiClickEncode，不立即上报
        if(pstXB->usPressCnt == pstXBCfg->usLongTime){ // 长按标记
            pstXB->ucMultiClickEncode |= (1<<(pstXB->ucClickCnt-1)); // bit(N-1)=第N击是长击
        }
    }
    goto _LabelXBEnd;
_LabelXBEventNotify:
    ...
    pstXB->ucMultiClickEncode = 0; // 重置多击事件标记
_LabelXBEnd:
