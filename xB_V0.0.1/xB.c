...
void vXB(stXBTypes * pstXB){
    ...
    if(uxCurVal == DE_NoPress){
        if(pstXB->ucReleasCnt <= pstXBCfg->usLongTime + pstXBCfg->ucHoldTime){
            pstXB->ucReleasCnt++;
        }
        if(pstXB->ucReleasCnt == ){;} //保持松开超过一定时间，则上报按键空闲事件
    ...
    goto _LabelXBEnd;
_LabelXBEventNotify:
    pstXB->pstXBCfg->pvEventHandler(uxNotifyVal, uiEvent);
_LabelXBEnd:
